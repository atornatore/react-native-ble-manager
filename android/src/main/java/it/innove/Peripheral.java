...
public class Peripheral extends BluetoothGattCallback {
	...
	public void connect(Callback callback, Activity activity, boolean autoConnect) {
		if (!connected) {
			...
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
				Log.d(BleManager.LOG_TAG, " Is Or Greater than M, autoConnect: " + autoConnect);
				...
				Runnable connectionRunnable = new Runnable() {
					...
					public void run() {
						try {
							gatt = device.connectGatt(activity, autoConnect, cb, BluetoothDevice.TRANSPORT_LE);
						} catch (NullPointerException e) {
							Log.d(BleManager.LOG_TAG, "Error while gatt.connect()");
						}

					}
				};

				mainHandler.post(connectionRunnable);
			...
			if (gatt != null) {
				Log.d(TAG, " so invoke callback");
				callback.invoke();
			} else {
				Log.d(TAG, " so invoke callback but BluetoothGatt is null");
	...
	public void onServicesDiscovered(BluetoothGatt gatt, int status) {
		...
		if (retrieveServicesCallback != null && gatt != null) {
	...
	public void onConnectionStateChange(BluetoothGatt gatta, int status, int newState) {
		...
		if (status != BluetoothGatt.GATT_SUCCESS) {
			...
			gatt.close();
			if(status==133){
				this.connect(connectCallback, reactContext.getCurrentActivity(), false);
				return;
			}
		...
		if (newState == BluetoothProfile.STATE_CONNECTED) {
			...
			if (device == null) {
				Log.e(BleManager.LOG_TAG, "Device received notification after disconnection.");
				Log.d(BleManager.LOG_TAG, "gatt.close()");
				gatt.close();
				return;
			}
			connected = true;
	...
	private void setNotify(UUID serviceUUID, UUID characteristicUUID, final Boolean notify, Callback callback) {
		...
		if ((characteristic.getProperties() & BluetoothGattCharacteristic.PROPERTY_NOTIFY) != 0) {
			Log.d(BleManager.LOG_TAG, "Characteristic " + characteristicUUID + " set NOTIFY "+ notify);
